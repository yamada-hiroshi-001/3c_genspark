...
st.markdown("""
GenSpark用3C分析プロンプトが出てくるだけのツールです。プロンプトと参照データをGensparkにペタりんして、３C分析スライドのたたきを作成します。
利用には下記のデータが必要です：
...
自分でテキスト書くよりは早いと思います。GENSPARK上で修正。補完等々行って最終的にPDFでDLしてそのまま使うか、PPTでDLしてテンプレートとか合わせていく（超めんどいが）のが良いと思います。
